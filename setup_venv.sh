...
## See following URL for author's commentary
## https://medium.com/@rafal.kedziorski
## https://medium.com/@rafal.kedziorski/automate-your-python-environment-a-bash-script-for-virtual-environment-setup-970888124c8e
...
if [ -n "$VIRTUAL_ENV" ]; then
    printf "[INFO] A virtual environment is already activated: %s\n" "${VIRTUAL_ENV}"
    return 0  # Use 'return' to prevent further execution
fi

# Check if the virtual environment already exists
if [ -d "$VENV_DIR" ]; then
    printf "[INFO] Virtual environment '%s' already exists. Activating it..." "${VENV_DIR}"
    activate_venv
else
    printf "[INFO] Virtual environment '%s' does not exist. Creating it..." "${VENV_DIR}"
    ...
    printf "\n[INFO] Virtual environment '%s' created successfully. Activating it..." "${VENV_DIR}"
    activate_venv
fi

# Verify that the virtual environment is activated
if [ -n "$VIRTUAL_ENV" ]; then
    printf "\n[INFO] Virtual environment '%s' is now active.\n" "${VENV_DIR}"
else
